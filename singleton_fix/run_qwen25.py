"""run_qwen25.py — EAP-screen → PP-verify pipeline for Qwen-2.5-3B.

Drives the existing model-agnostic pipeline (run_lds / run_pp_topheads / triangulation)
for the Qwen-2.5-3B base/instruct pair. Produces the same file layout as the Llama and
Gemma runs and writes results/qwen25_3b_summary_st.json at the end.

Usage:
  # Base model — both prompt types + summary
  python singleton_fix/run_qwen25.py --variant base

  # Instruct model
  python singleton_fix/run_qwen25.py --variant instruct

  # Both models back-to-back (run base first, then instruct)
  python singleton_fix/run_qwen25.py --variant both

  # Smoke test (16-prompt subset, skip summary)
  python singleton_fix/run_qwen25.py --variant base --n-subset 16

  # Build summary only (after both cells exist)
  python singleton_fix/run_qwen25.py --summary-only

All outputs → results/. Files are skipped if they already exist (resumable).

Access prereq: none -- Qwen-2.5-3B and Qwen-2.5-3B-Instruct are public repos;
  HF_TOKEN is not required (anonymous download works).
"""
...
# This file lives one directory below the project root (singleton_fix/),
# so _ROOT must go up TWO levels to reach foundation.py/contract.py/path_patching/
# -- mirrors path_patching/path_patching.py's own _ROOT computation.
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _ROOT not in sys.path:
    sys.path.insert(0, _ROOT)

# Path patching lives in its own sub-package at the project root.
...
FAMILY = "qwen25_3b"  # per-family results folder, mirrors singleton_fix.run_single_token_filter_all
...
def _check_tokenizer(model) -> None:
    """Verify _answer_ids round-trips correctly for Qwen's BPE tokenizer."""
...
# ---------------------------------------------------------------------------
# Summary builder (mirrors llama32_3b_summary.json / gemma3_4b_summary.json schema)
# ---------------------------------------------------------------------------
...
def build_summary() -> None:
    out_path = RESULTS_DIR / f"qwen25_3b_summary_{SET_TAG}.json"
    summary  = {
        "experiment": "Qwen-2.5-3B EAP-screen -> PP-verify",
...
def run_variant(
    ...
    print(f"\n{'='*70}")
    print(f"[qwen25] variant={variant!r}  tag={tag!r}  model={model_name!r}")
    print(f"{'='*70}")

    print(f"[qwen25] Loading model (dtype={DTYPE}) ...")
    kwargs = {}
    if hf_model:
    ...
    print(
        f"[qwen25] cfg: n_layers={model.cfg.n_layers} n_heads={model.cfg.n_heads} "
    ...
    if "lds" in steps:
        for ptype in PROMPT_TYPES:
            ...
            if n_subset is not None:
                ...
                rng = _random.Random(seed)
                prompts = rng.sample(prompts, min(n_subset, len(prompts)))
                print(f"[qwen25] EAP smoke: using {len(prompts)} prompts for {ptype}")
    ...
    if "pp" in steps:
        ...
        for ptype in PP_PROMPT_TYPES:
            ...
            if not eap_path.exists():
                print(
                    f"[qwen25] ERROR: {eap_path} missing -- run LDS for this variant "
    ...
    print(f"[qwen25] variant={variant!r} done (steps={steps}).")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="EAP-screen → PP-verify for Qwen-2.5-3B base/instruct.",
    ...
    parser.add_argument(
        ...
        help="Skip all model runs; just rebuild qwen25_3b_summary_st.json from existing files.",
    )
    parser.add_argument(
    ...
    if args.n_subset is None and args.split == "all":
        ...
        print("[qwen25] Smoke/split run complete — skipping auto summary.")
