"""run_llama32.py — EAP-screen → PP-verify pipeline for Llama-3.2-3B.

Drives the existing model-agnostic pipeline (run_lds / run_pp_topheads / triangulation)
for the Llama-3.2-3B base/instruct pair. Produces the same file layout as the Qwen run
and writes results/llama32_3b_summary.json at the end.

Usage:
  # Base model — both prompt types + summary
  python run_llama32.py --variant base

  # Instruct model
  python run_llama32.py --variant instruct

  # Both models back-to-back (run base first, then instruct)
  python run_llama32.py --variant both

  # Smoke test (16-prompt subset, skip summary)
  python run_llama32.py --variant base --n-subset 16

  # Build summary only (after both cells exist)
  python run_llama32.py --summary-only

All outputs → results/. Files are skipped if they already exist (resumable).

Access prereq: HF_TOKEN in .env must belong to an account that accepted the
  Llama-3.2 license on both meta-llama/Llama-3.2-3B and meta-llama/Llama-3.2-3B-Instruct.
"""
...
FAMILY = "llama32_3b"  # per-family results folder, mirrors singleton_fix.run_single_token_filter_all
...
# ---------------------------------------------------------------------------
# Tokenizer round-trip sanity (§4 of the spec — run before attribution)
# ---------------------------------------------------------------------------

def _check_tokenizer(model) -> None:
    """Verify _answer_ids round-trips correctly for Llama's BPE tokenizer."""
    from foundation import _answer_ids
    probe = "soccer"
    ids = _answer_ids(model, probe)
    decoded = model.tokenizer.decode(ids.tolist())
    expected_suffix = probe  # the decoded form should contain the probe word
    if probe not in decoded:
        raise RuntimeError(
            f"[tokenizer-check] FAIL: _answer_ids('{probe}') decoded to {decoded!r}. "
            f"The leading-space BPE convention is not round-tripping correctly for this "
            f"tokenizer. Fix _answer_ids in foundation.py before running attribution."
        )
    print(
        f"[tokenizer-check] OK: _answer_ids('{probe}') → {ids.tolist()} → {decoded!r} "
        f"(contains '{probe}')"
    )
    print(f"[tokenizer-check] ids are in-vocab (max={ids.max().item()} < d_vocab={model.cfg.d_vocab}): "
          f"{'OK' if ids.max().item() < model.cfg.d_vocab else 'FAIL'}")
    if ids.max().item() >= model.cfg.d_vocab:
        raise RuntimeError("[tokenizer-check] FAIL: answer id out of vocab range.")
...
# ---------------------------------------------------------------------------
# Summary builder (mirrors qwen25_3b_summary.json schema)
# ---------------------------------------------------------------------------
...
def _spearman(xs: List[float], ys: List[float]) -> Tuple[float, float]:
    ...
    # Two-tailed t-test p-value (t = rho * sqrt((n-2)/(1-rho^2)))
    import math
    if abs(rho) >= 1.0:
        return rho, 0.0
    t  = rho * math.sqrt((n - 2) / (1 - rho ** 2))
    # CDF approximation via regularised incomplete beta (math module only)
    # Use the simple normal approximation for large n; for n=20 the error is <0.01
    # (same approach as triangulation.py which verified against scipy to 4 dp)
    z  = abs(t) / math.sqrt(1 + t * t / (n - 2))  # Wilson-Hilferty not needed at n≥20
    # Gaussian CDF via math.erfc
...
def _build_cell_summary(tag: str, ptype: str) -> Optional[Dict]:
    ...
    eap_data  = _load_eap(eap_path)
    selected  = pp_data["selected_heads"]  # already the ~20 top heads
...
def build_summary() -> None:
    out_path = RESULTS_DIR / f"llama32_3b_summary_{SET_TAG}.json"
    summary  = {
        "experiment": "Llama-3.2-3B EAP-screen -> PP-verify",
...
def run_variant(
    ...
    print(f"\n{'='*70}")
    print(f"[llama32] variant={variant!r}  tag={tag!r}  model={model_name!r}")
    print(f"{'='*70}")

    print(f"[llama32] Loading model (dtype={DTYPE}) ...")
    # Prefer a pre-downloaded local directory over a live HF fetch.
    # hf_hub_download saves to ./llama32_instruct or ./llama32_base relative
    # to _ROOT when dl_llama.py is run from the project root.
    if hf_model is None:
        _local_dirs = {
            "instruct": os.path.join(_ROOT, "llama32_instruct"),
            "base":     os.path.join(_ROOT, "llama32_base"),
        }
        _local = _local_dirs.get(variant)
        if _local and os.path.isdir(_local):
            hf_model = _local
            print(f"[llama32] Using local model dir: {_local}")
    kwargs = {}
    if hf_model:
    ...
    print(
        f"[llama32] cfg: n_layers={model.cfg.n_layers} n_heads={model.cfg.n_heads} "
        f"d_vocab={model.cfg.d_vocab} dtype={next(model.parameters()).dtype}"
    )

    # Tokenizer sanity (§4 — fast check, stops early if wiring is broken)
    ...
    if "lds" in steps:
        for ptype in PROMPT_TYPES:
            ...
            if n_subset is not None:
                ...
                rng = _random.Random(seed)
                prompts = rng.sample(prompts, min(n_subset, len(prompts)))
                print(f"[llama32] EAP smoke: using {len(prompts)} prompts for {ptype}")
    ...
    if "pp" in steps:
        # Free GPU memory before PP (PP reloads no new weights but its own peak is lower)
        torch.cuda.empty_cache() if torch.cuda.is_available() else None
        for ptype in PP_PROMPT_TYPES:
            eap_path = RESULTS_DIR / f"lds2_{tag}_{ptype}_{set_tag}_eap.json"
            if not eap_path.exists():
                print(
                    f"[llama32] ERROR: {eap_path} missing -- run LDS for this variant "
    ...
    # (run_all_lds.py / run_all_pathpatching.py) loads the next one --
    # don't rely on GC alone, same defense-in-depth as
    # modal_single_token_filter.py. Matters most on smaller GPUs (not
    # everyone has 24GB) where a lingering ~6-8GB model would OOM the next load.
    del model
    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    print(f"[llama32] variant={variant!r} done (steps={steps}).")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="EAP-screen → PP-verify for Llama-3.2-3B base/instruct.",
    ...
    parser.add_argument(
        ...
        help="Skip all model runs; just rebuild llama32_3b_summary.json from existing files.",
    )
    parser.add_argument(
    ...
    if args.n_subset is None and args.split == "all":
        ...
        print("[llama32] Smoke run complete — skipping summary (subset only).")
